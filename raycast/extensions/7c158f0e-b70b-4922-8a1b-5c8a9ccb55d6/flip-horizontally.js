...
`);return t?t.originalMessage=t.message:t=l,t.message=T,t.shortMessage=f,t.command=o,t.exitCode=s,t.signal=a,t.stdout=r,t.stderr=e,"bufferedData"in t&&delete t.bufferedData,t}function He({stdout:r,stderr:e,error:t,exitCode:a,signal:s,timedOut:o,command:n,options:i,parentError:l}){if(t||a!==0||s!==null)throw Be({error:t,exitCode:a,signal:s,stdout:r,stderr:e,command:n,timedOut:o,options:i,parentError:l});return r}async function $(r,e,t){if(process.platform!=="darwin")throw new Error("AppleScript is only supported on macOS");let{humanReadableOutput:a,language:s,timeout:o,...n}=Array.isArray(e)?t||{}:e||{},i=a!==!1?[]:["-ss"];s==="JavaScript"&&i.push("-l","JavaScript"),Array.isArray(e)&&i.push("-",...e);let l=ie.default.spawn("osascript",i,{...n,env:{PATH:"/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin"}}),u=We(l,{timeout:o??1e4});l.stdin.end(r);let[{error:y,exitCode:f,signal:T,timedOut:M},S,R]=await je(l,{encoding:"utf8"},u),Re=se({stripFinalNewline:!0},S),Ee=se({stripFinalNewline:!0},R);return He({stdout:Re,stderr:Ee,error:y,exitCode:f,signal:T,timedOut:M,command:"osascript",options:t,parentError:new Error})}var F=require("child_process"),m=require("@raycast/api");var ue=require("fs");async function Ge(){let r="";try{r=(0,F.execSync)(`/bin/zsh -lc 'realpath "$(which brew)"'`).toString().trim()}catch(e){console.error(e)}if(r==="")return await(0,m.showToast)({title:"Homebrew is required to install the AVIF encoder.",message:"Please install Homebrew and try again. Visit https://brew.sh for more information. Once Homebrew is installed, run the command `brew install libavif` to install the AVIF encoder manually (Otherwise, this command will be run automatically).",style:m.Toast.Style.Failure}),!1;if(await(0,m.confirmAlert)({title:"Install AVIF Encoder",message:"The libavif Homebrew formula is required to convert images to/from AVIF format. Would you like to install it now?",primaryAction:{title:"Install"}})){let e=await(0,m.showToast)({title:"Installing AVIF Encoder...",style:m.Toast.Style.Animated});try{if((0,F.execSync)(`/bin/zsh -ilc '${r} install --quiet libavif || true'`),!Ke())throw new Error("The avifenc binary has not been added to the user's $PATH");return e.title="AVIF Encoder installed successfully!",e.style=m.Toast.Style.Success,!0}catch(t){console.error(t),b("Failed to install AVIF Encoder.",t,e,"If you previously attempted to install libavif or avifenc, please run `brew doctor` followed by `brew cleanup` and try again.")}}return await(0,m.showToast)({title:"AVIF Encoder not installed.",style:m.Toast.Style.Failure}),!1}async function Ke(){let t=!1,a=0;for(;!t&&a<7;){let s=(0,F.execSync)("/bin/zsh -lc 'command -v avifenc'").toString().trim();if((0,ue.existsSync)(s)){t=!0;break}await new Promise(o=>setTimeout(o,1e3)),a++}return t}async function J(){let r=await m.LocalStorage.getItem("avifEncoderPath"),e=await m.LocalStorage.getItem("avifDecoderPath");if(!r||!e)try{r=(0,F.execSync)(`/bin/zsh -lc 'realpath "$(which avifenc)"'`).toString().trim(),e=(0,F.execSync)(`/bin/zsh -lc 'realpath "$(which avifdec)"'`).toString().trim()}catch(t){if(await Ge())try{return await J()}catch(a){console.error(a),b("AVIF Encoder not found.",a,void 0,"Please install the libavif Homebrew formula manually and try again.")}else b("AVIF Encoder not found.",t,void 0,"Please install the libavif Homebrew formula and try again.")}return{encoderPath:r,decoderPath:e}}var fe=async()=>$(`use framework "AppKit"
      ...
      end if`)};var P=w(require("path"));var L=require("child_process");function v(r,e){let t=e?.command,a=e?.language,s=[...e?.leadingArgs?.map(f=>f.toString())||[]],o=e?.trailingArgs||[];!t&&(a===void 0||a==="AppleScript"||a==="JXA")&&(t="/usr/bin/osascript",s.push("-l",a==="JXA"?"JavaScript":"AppleScript",...r.startsWith("/")?[]:["-e"],r,...o.map(f=>f.toString())));let n=process.env;if(e?.command&&(n.PATH=`${n.PATH}:${(0,L.execSync)(`$(/bin/bash -lc 'echo $SHELL') -lc 'echo "$PATH"'`).toString()}`,t=e.command,s.push(r,...o.map(f=>f.toString()))),!t)throw new Error("No command specified.");let i="",l=f=>{console.log(f)},u=(0,L.spawn)(t,s,{env:n});return e?.logDebugMessages&&console.log(`Running shell command "${t} ${s.join(" ")}"`),u.stdout?.on("data",f=>{i+=f.toString(),e?.logIntermediateOutput&&console.log(`Data from script: ${i}`)}),u.stderr?.on("data",f=>{e?.stderrCallback&&e.stderrCallback(f.toString()),e?.logErrors&&console.error(f.toString())}),u.stdin.on("error",f=>{e?.logErrors&&console.error(`Error writing to stdin: ${f}`)}),l=async f=>{f?.length>0&&(u.stdin.cork(),u.stdin.write(`${f}\r
`),process.nextTick(()=>u.stdin.uncork()),e?.logSentMessages&&console.log(`Sent message: ${f}`))},{data:(async()=>new Promise((f,T)=>{let M=e?.timeout?setTimeout(()=>{try{u.kill()}catch(S){e?.logErrors&&console.error(`Error killing process: ${S}`)}return e?.logErrors&&console.error("Script timed out"),u.stdin.end(),u.kill(),T("Script timed out")},e?.timeout):void 0;u.on("close",S=>{if(S!==0)return e?.logErrors&&console.error(`Script exited with code ${S}`),u.stdin.end(),u.kill(),T(`Script exited with code ${S}`);clearTimeout(M);let R;try{R=JSON.parse(i)}catch{R=i.trim()}return e?.logFinalOutput&&console.log(`Script output: ${R}`),f(R)})}))(),sendMessage:l}}var I=require("@raycast/api");async function pe(){let r=P.default.join(I.environment.assetsPath,"scripts","finder.scpt"),e=await v(r,{language:"AppleScript",stderrCallback:t=>b("Finder Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function he(){let r=P.default.join(I.environment.assetsPath,"scripts","houdahSpot.scpt"),e=await v(r,{language:"AppleScript",stderrCallback:t=>b("HoudahSpot Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function me(){let r=P.default.join(I.environment.assetsPath,"scripts","neofinder.scpt"),e=await v(r,{language:"AppleScript",stderrCallback:t=>b("NeoFinder Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function ge(){let r=P.default.join(I.environment.assetsPath,"scripts","pathfinder.scpt"),e=await v(r,{language:"JXA",stderrCallback:t=>b("Path Finder Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function be(){let r=P.default.join(I.environment.assetsPath,"scripts","qspace.scpt"),e=await v(r,{language:"JXA",stderrCallback:t=>b("QSpace Pro Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function ye(){let r=P.default.join(I.environment.assetsPath,"scripts","forklift-beta.scpt"),e=await v(r,{language:"JXA",stderrCallback:t=>b("ForkLift Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}var X=async(r,e)=>{let t=d.default.join(g.tmpdir(),`${r}.${e}`);return{path:t,[Symbol.asyncDispose]:async()=>{p.existsSync(t)&&await p.promises.rm(t,{recursive:!0})}}};var ke=async()=>{let e=(await c.LocalStorage.getItem("itemsToRemove")??"").toString().split(", ");for(let t of e)p.existsSync(t)&&await p.promises.rm(t,{recursive:!0});await c.LocalStorage.removeItem("itemsToRemove")},Se=async()=>{let r=[],t=(0,c.getPreferenceValues)().inputMethod,a=!1;if(t=="Clipboard")try{let n=(await fe()).split(", ").reduce((i,l,u)=>(u==0||l.startsWith("/")?i.push(l):i[i.length-1]=`${i.at(-1)??""}${l}`,i),[]);if(await c.LocalStorage.setItem("itemsToRemove",n.join(", ")),n.filter(i=>i.trim().length>0).length>0)return n}catch(n){console.error(`Couldn't get images from clipboard: ${n}`),a=!0}let s=t;try{s=(await(0,c.getFrontmostApplication)()).name}catch(n){console.error(`Couldn't get frontmost application: ${n}`)}try{(t=="Path Finder"||s=="Path Finder")&&(r=await ge())}catch(n){console.error(`Couldn't get images from Path Finder: ${n}`),a=!0}try{(t=="NeoFinder"||s=="NeoFinder")&&(r=await me())}catch(n){console.error(`Couldn't get images from NeoFinder: ${n}`),a=!0}try{(t=="HoudahSpot"||s=="HoudahSpot")&&(r=await he())}catch(n){console.error(`Couldn't get images from HoudahSpot: ${n}`),a=!0}try{(t=="QSpace"||s=="QSpace Pro"||s=="QSpace")&&(r=await be())}catch(n){console.error(`Couldn't get images from ${s}: ${n}`),a=!0}try{(t=="ForkLift"||s=="ForkLift")&&(r=await ye())}catch(n){console.error(`Couldn't get images from ForkLift: ${n}`),a=!0}if(r.length>0)return r.filter((n,i)=>r.indexOf(n)===i);let o=await pe();return s=="Finder"||t=="Finder"||a?r=o:o.forEach(n=>{n.split("/").at(-2)=="Desktop"&&!r.includes(n)&&r.push(n)}),r.filter((n,i)=>r.indexOf(n)===i)},Z=async r=>{let e="Finder";try{e=(await(0,c.getFrontmostApplication)()).name}catch(a){console.error(`Couldn't get frontmost application: : ${a}`)}let t=(0,c.getPreferenceValues)();t.imageResultHandling=="copyToClipboard"?(await de(r),$e(r)):t.imageResultHandling=="openInPreview"?(await Ze(r),$e(r)):t.inputMethod=="NeoFinder"||e=="NeoFinder"?await(0,c.showInFinder)(r[0]):(t.inputMethod=="HoudahSpot"||e=="HoudahSpot")&&await(0,c.showInFinder)(r[0])},Je=async()=>(g.cpus()[0].model.includes("Apple")?"arm":"x86")=="arm"?((0,k.execSync)(`chmod +x ${c.environment.assetsPath}/webp/arm/dwebp`),(0,k.execSync)(`chmod +x ${c.environment.assetsPath}/webp/arm/cwebp`),p.existsSync(`${c.environment.assetsPath}/webp/x86/dwebp`)&&await p.promises.rm(`${c.environment.assetsPath}/webp/x86/dwebp`),p.existsSync(`${c.environment.assetsPath}/webp/x86/cwebp`)&&await p.promises.rm(`${c.environment.assetsPath}/webp/x86/cwebp`),[`${c.environment.assetsPath}/webp/arm/dwebp`,`${c.environment.assetsPath}/webp/arm/cwebp`]):((0,k.execSync)(`chmod +x ${c.environment.assetsPath}/webp/x86/dwebp`),(0,k.execSync)(`chmod +x ${c.environment.assetsPath}/webp/x86/cwebp`),p.existsSync(`${c.environment.assetsPath}/webp/arm/dwebp`)&&await p.promises.rm(`${c.environment.assetsPath}/webp/arm/dwebp`),p.existsSync(`${c.environment.assetsPath}/webp/arm/cwebp`)&&await p.promises.rm(`${c.environment.assetsPath}/webp/arm/cwebp`),[`${c.environment.assetsPath}/webp/x86/dwebp`,`${c.environment.assetsPath}/webp/x86/cwebp`]),xe=async(r,e)=>{var i=[];try{let t=(0,c.getPreferenceValues)();let a=O(i,await X("tmp","png"),!0);let s=(await C([e]))[0];let[o,n]=await Je();(0,k.execSync)(`${o} "${e}" -o "${a.path}" && ${r} "${a.path}" && ${n} ${t.useLosslessConversion?"-lossless":""} "${a.path}" -o "${s}"`);return s}catch(l){var u=l,y=!0}finally{var f=_(i,u,y);f&&await f}},ve=async(r,e)=>{var i=[];try{let t=(0,c.getPreferenceValues)();let a=O(i,await X("tmp","png"),!0);let s=(await C([e]))[0];let{encoderPath:o,decoderPath:n}=await J();(0,k.execSync)(`${n} "${e}" "${a.path}" && ${r} "${a.path}" && ${o} ${t.useLosslessConversion?"-s 0 --min 0 --max 0 --minalpha 0 --maxalpha 0 --qcolor 100 --qalpha 100":""}  "${a.path}" "${s}"`);return s}catch(l){var u=l,y=!0}finally{var f=_(i,u,y);f&&await f}},Pe=async(r,e)=>{var s=[];try{let t=O(s,await X("tmp","bmp"),!0);let a=(await C([e]))[0];await Xe("BMP",e,t.path);(0,k.execSync)(`chmod +x ${c.environment.assetsPath}/potrace/potrace`);(0,k.execSync)(`${r} "${t.path}" && ${c.environment.assetsPath}/potrace/potrace -s --tight -o "${a}" "${t.path}"`);return a}catch(o){var n=o,i=!0}finally{var l=_(s,n,i);l&&await l}},Xe=async(r,e,t)=>$(`use framework "Foundation"
  use scripting additions
  ...
  pngData's writeToFile:"${t}" atomically:false`);var Ie=async(r,e)=>{let t=(0,c.getPreferenceValues)(),a=r;t.imageResultHandling=="saveToDownloads"?a=d.default.join(g.homedir(),"Downloads",d.default.basename(a)):t.imageResultHandling=="saveToDesktop"?a=d.default.join(g.homedir(),"Desktop",d.default.basename(a)):(t.imageResultHandling=="copyToClipboard"||t.imageResultHandling=="openInPreview")&&(a=d.default.join(g.tmpdir(),d.default.basename(a)));let s=2;for(;p.existsSync(a)&&g.tmpdir()!=d.default.dirname(a);)a=d.default.join(d.default.dirname(a),d.default.basename(a,d.default.extname(a))+` (${s})${d.default.extname(a)}`),s++;let o=e==0?`(transform's scaleXBy:-1 yBy:1)
