...
`),process.nextTick(()=>u.stdin.uncork()),e?.logSentMessages&&console.log(`Sent message: ${l}`))},{data:(async()=>new Promise((l,C)=>{let R=e?.timeout?setTimeout(()=>{try{u.kill()}catch(F){e?.logErrors&&console.error(`Error killing process: ${F}`)}return e?.logErrors&&console.error("Script timed out"),u.stdin.end(),u.kill(),C("Script timed out")},e?.timeout):void 0;u.on("close",F=>{if(F!==0)return e?.logErrors&&console.error(`Script exited with code ${F}`),u.stdin.end(),u.kill(),C(`Script exited with code ${F}`);clearTimeout(R);let v;try{v=JSON.parse(c)}catch{v=c.trim()}return e?.logFinalOutput&&console.log(`Script output: ${v}`),l(v)})}))(),sendMessage:p}}var I=require("@raycast/api");async function ce(){let r=k.default.join(I.environment.assetsPath,"scripts","finder.scpt"),e=await S(r,{language:"AppleScript",stderrCallback:t=>g("Finder Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function le(){let r=k.default.join(I.environment.assetsPath,"scripts","houdahSpot.scpt"),e=await S(r,{language:"AppleScript",stderrCallback:t=>g("HoudahSpot Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function ue(){let r=k.default.join(I.environment.assetsPath,"scripts","neofinder.scpt"),e=await S(r,{language:"AppleScript",stderrCallback:t=>g("NeoFinder Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function de(){let r=k.default.join(I.environment.assetsPath,"scripts","pathfinder.scpt"),e=await S(r,{language:"JXA",stderrCallback:t=>g("Path Finder Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function pe(){let r=k.default.join(I.environment.assetsPath,"scripts","qspace.scpt"),e=await S(r,{language:"JXA",stderrCallback:t=>g("QSpace Pro Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function fe(){let r=k.default.join(I.environment.assetsPath,"scripts","forklift-beta.scpt"),e=await S(r,{language:"JXA",stderrCallback:t=>g("ForkLift Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}var me=async()=>{let e=(await d.LocalStorage.getItem("itemsToRemove")??"").toString().split(", ");for(let t of e)x.existsSync(t)&&await x.promises.rm(t,{recursive:!0});await d.LocalStorage.removeItem("itemsToRemove")},ge=async()=>{let r=[],t=(0,d.getPreferenceValues)().inputMethod,a=!1;if(t=="Clipboard")try{let n=(await se()).split(", ").reduce((c,p,u)=>(u==0||p.startsWith("/")?c.push(p):c[c.length-1]=`${c.at(-1)??""}${p}`,c),[]);if(await d.LocalStorage.setItem("itemsToRemove",n.join(", ")),n.filter(c=>c.trim().length>0).length>0)return n}catch(n){console.error(`Couldn't get images from clipboard: ${n}`),a=!0}let s=t;try{s=(await(0,d.getFrontmostApplication)()).name}catch(n){console.error(`Couldn't get frontmost application: ${n}`)}try{(t=="Path Finder"||s=="Path Finder")&&(r=await de())}catch(n){console.error(`Couldn't get images from Path Finder: ${n}`),a=!0}try{(t=="NeoFinder"||s=="NeoFinder")&&(r=await ue())}catch(n){console.error(`Couldn't get images from NeoFinder: ${n}`),a=!0}try{(t=="HoudahSpot"||s=="HoudahSpot")&&(r=await le())}catch(n){console.error(`Couldn't get images from HoudahSpot: ${n}`),a=!0}try{(t=="QSpace"||s=="QSpace Pro"||s=="QSpace")&&(r=await pe())}catch(n){console.error(`Couldn't get images from ${s}: ${n}`),a=!0}try{(t=="ForkLift"||s=="ForkLift")&&(r=await fe())}catch(n){console.error(`Couldn't get images from ForkLift: ${n}`),a=!0}if(r.length>0)return r.filter((n,c)=>r.indexOf(n)===c);let o=await ce();return s=="Finder"||t=="Finder"||a?r=o:o.forEach(n=>{n.split("/").at(-2)=="Desktop"&&!r.includes(n)&&r.push(n)}),r.filter((n,c)=>r.indexOf(n)===c)},be=async r=>{let e="Finder";try{e=(await(0,d.getFrontmostApplication)()).name}catch(a){console.error(`Couldn't get frontmost application: : ${a}`)}let t=(0,d.getPreferenceValues)();t.imageResultHandling=="copyToClipboard"?(await oe(r),he(r)):t.imageResultHandling=="openInPreview"?(await We(r),he(r)):t.inputMethod=="NeoFinder"||e=="NeoFinder"?await(0,d.showInFinder)(r[0]):(t.inputMethod=="HoudahSpot"||e=="HoudahSpot")&&await(0,d.showInFinder)(r[0])};var We=async r=>{let e=Array.isArray(r)?r:[r],t=e.some(a=>f.default.extname(a)==".svg");await m(`use framework "Foundation"
  ...
  end tell`)},ye=async(r,e=!1,t=void 0)=>{let a=(0,d.getPreferenceValues)(),s=await Be(r[0]);return r.map(o=>{let n=o;if(a.imageResultHandling=="saveToDownloads"?n=f.default.join(b.homedir(),"Downloads",f.default.basename(n)):a.imageResultHandling=="saveToDesktop"?n=f.default.join(b.homedir(),"Desktop",f.default.basename(n)):(a.imageResultHandling=="saveInContainingFolder"||a.imageResultHandling=="replaceOriginal")&&(a.inputMethod=="Clipboard"||e)?n=f.default.join(s,f.default.basename(n)):(a.imageResultHandling=="copyToClipboard"||a.imageResultHandling=="openInPreview")&&(n=f.default.join(b.tmpdir(),f.default.basename(n))),n=t?n.replace(f.default.extname(n),`.${t}`):n,a.imageResultHandling!="replaceOriginal"&&b.tmpdir()!=f.default.dirname(n)){let c=2;for(;x.existsSync(n);)n=f.default.join(f.default.dirname(n),f.default.basename(n,f.default.extname(n))+`-${c}${f.default.extname(n)}`),c++}return n})},g=async(r,e,t,a)=>{console.error(e),t?(t.title=r,t.message=a??e.message,t.style=d.Toast.Style.Failure,t.primaryAction={title:"Copy Error",onAction:async()=>{await d.Clipboard.copy(e.message)}}):t=await(0,d.showToast)({title:r,message:a??e.message,style:d.Toast.Style.Failure,primaryAction:{title:"Copy Error",onAction:async()=>{await d.Clipboard.copy(e.message)}}})};var Fe=r=>{let e=b.homedir();if(r.startsWith("~"))return r.replace(/^~(?=$|\/|\\)/,e);let t=/(\/Users\/.*?)\/.*/,a=r.match(t);return a?r.replace(a[1],e):r};async function M(r,e){let t=[],a=r.map(s=>Fe(s));for(let s of a){let o=(await ye([s],!1,s.endsWith(".pdf")?"pdf":"png"))[0];await ne(s,o,e),t.push(o)}return await be(t),t}async function Ve({filters:r,imagePaths:e}){let t=e?.length?e:await ge(),a=r.map(o=>ie().find(n=>n.name===o)),s=new Array;for(let o of a){if(!o)throw new Error(`Filter "${o}" not found.`);t=s?.length?s:t,s=await M(t,o)}return await me(),s}
