...
`);return t?t.originalMessage=t.message:t=l,t.message=E,t.shortMessage=d,t.command=i,t.exitCode=s,t.signal=a,t.stdout=r,t.stderr=e,"bufferedData"in t&&delete t.bufferedData,t}function je({stdout:r,stderr:e,error:t,exitCode:a,signal:s,timedOut:i,command:n,options:o,parentError:l}){if(t||a!==0||s!==null)throw Me({error:t,exitCode:a,signal:s,stdout:r,stderr:e,command:n,timedOut:i,options:o,parentError:l});return r}async function $(r,e,t){if(process.platform!=="darwin")throw new Error("AppleScript is only supported on macOS");let{humanReadableOutput:a,language:s,timeout:i,...n}=Array.isArray(e)?t||{}:e||{},o=a!==!1?[]:["-ss"];s==="JavaScript"&&o.push("-l","JavaScript"),Array.isArray(e)&&o.push("-",...e);let l=ne.default.spawn("osascript",o,{...n,env:{PATH:"/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin"}}),u=Oe(l,{timeout:i??1e4});l.stdin.end(r);let[{error:b,exitCode:d,signal:E,timedOut:M},S,R]=await Ne(l,{encoding:"utf8"},u),Pe=ae({stripFinalNewline:!0},S),Ie=ae({stripFinalNewline:!0},R);return je({stdout:Pe,stderr:Ie,error:b,exitCode:d,signal:E,timedOut:M,command:"osascript",options:t,parentError:new Error})}var F=require("child_process"),m=require("@raycast/api");var ce=require("fs");async function Ve(){let r="";try{r=(0,F.execSync)(`/bin/zsh -lc 'realpath "$(which brew)"'`).toString().trim()}catch(e){console.error(e)}if(r==="")return await(0,m.showToast)({title:"Homebrew is required to install the AVIF encoder.",message:"Please install Homebrew and try again. Visit https://brew.sh for more information. Once Homebrew is installed, run the command `brew install libavif` to install the AVIF encoder manually (Otherwise, this command will be run automatically).",style:m.Toast.Style.Failure}),!1;if(await(0,m.confirmAlert)({title:"Install AVIF Encoder",message:"The libavif Homebrew formula is required to convert images to/from AVIF format. Would you like to install it now?",primaryAction:{title:"Install"}})){let e=await(0,m.showToast)({title:"Installing AVIF Encoder...",style:m.Toast.Style.Animated});try{if((0,F.execSync)(`/bin/zsh -ilc '${r} install --quiet libavif || true'`),!ze())throw new Error("The avifenc binary has not been added to the user's $PATH");return e.title="AVIF Encoder installed successfully!",e.style=m.Toast.Style.Success,!0}catch(t){console.error(t),y("Failed to install AVIF Encoder.",t,e,"If you previously attempted to install libavif or avifenc, please run `brew doctor` followed by `brew cleanup` and try again.")}}return await(0,m.showToast)({title:"AVIF Encoder not installed.",style:m.Toast.Style.Failure}),!1}async function ze(){let t=!1,a=0;for(;!t&&a<7;){let s=(0,F.execSync)("/bin/zsh -lc 'command -v avifenc'").toString().trim();if((0,ce.existsSync)(s)){t=!0;break}await new Promise(i=>setTimeout(i,1e3)),a++}return t}async function J(){let r=await m.LocalStorage.getItem("avifEncoderPath"),e=await m.LocalStorage.getItem("avifDecoderPath");if(!r||!e)try{r=(0,F.execSync)(`/bin/zsh -lc 'realpath "$(which avifenc)"'`).toString().trim(),e=(0,F.execSync)(`/bin/zsh -lc 'realpath "$(which avifdec)"'`).toString().trim()}catch(t){if(await Ve())try{return await J()}catch(a){console.error(a),y("AVIF Encoder not found.",a,void 0,"Please install the libavif Homebrew formula manually and try again.")}else y("AVIF Encoder not found.",t,void 0,"Please install the libavif Homebrew formula and try again.")}return{encoderPath:r,decoderPath:e}}var le=async()=>$(`use framework "AppKit"
      ...
      end if`)};var P=w(require("path"));var L=require("child_process");function v(r,e){let t=e?.command,a=e?.language,s=[...e?.leadingArgs?.map(d=>d.toString())||[]],i=e?.trailingArgs||[];!t&&(a===void 0||a==="AppleScript"||a==="JXA")&&(t="/usr/bin/osascript",s.push("-l",a==="JXA"?"JavaScript":"AppleScript",...r.startsWith("/")?[]:["-e"],r,...i.map(d=>d.toString())));let n=process.env;if(e?.command&&(n.PATH=`${n.PATH}:${(0,L.execSync)(`$(/bin/bash -lc 'echo $SHELL') -lc 'echo "$PATH"'`).toString()}`,t=e.command,s.push(r,...i.map(d=>d.toString()))),!t)throw new Error("No command specified.");let o="",l=d=>{console.log(d)},u=(0,L.spawn)(t,s,{env:n});return e?.logDebugMessages&&console.log(`Running shell command "${t} ${s.join(" ")}"`),u.stdout?.on("data",d=>{o+=d.toString(),e?.logIntermediateOutput&&console.log(`Data from script: ${o}`)}),u.stderr?.on("data",d=>{e?.stderrCallback&&e.stderrCallback(d.toString()),e?.logErrors&&console.error(d.toString())}),u.stdin.on("error",d=>{e?.logErrors&&console.error(`Error writing to stdin: ${d}`)}),l=async d=>{d?.length>0&&(u.stdin.cork(),u.stdin.write(`${d}\r
`),process.nextTick(()=>u.stdin.uncork()),e?.logSentMessages&&console.log(`Sent message: ${d}`))},{data:(async()=>new Promise((d,E)=>{let M=e?.timeout?setTimeout(()=>{try{u.kill()}catch(S){e?.logErrors&&console.error(`Error killing process: ${S}`)}return e?.logErrors&&console.error("Script timed out"),u.stdin.end(),u.kill(),E("Script timed out")},e?.timeout):void 0;u.on("close",S=>{if(S!==0)return e?.logErrors&&console.error(`Script exited with code ${S}`),u.stdin.end(),u.kill(),E(`Script exited with code ${S}`);clearTimeout(M);let R;try{R=JSON.parse(o)}catch{R=o.trim()}return e?.logFinalOutput&&console.log(`Script output: ${R}`),d(R)})}))(),sendMessage:l}}var I=require("@raycast/api");async function fe(){let r=P.default.join(I.environment.assetsPath,"scripts","finder.scpt"),e=await v(r,{language:"AppleScript",stderrCallback:t=>y("Finder Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function de(){let r=P.default.join(I.environment.assetsPath,"scripts","houdahSpot.scpt"),e=await v(r,{language:"AppleScript",stderrCallback:t=>y("HoudahSpot Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function pe(){let r=P.default.join(I.environment.assetsPath,"scripts","neofinder.scpt"),e=await v(r,{language:"AppleScript",stderrCallback:t=>y("NeoFinder Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function he(){let r=P.default.join(I.environment.assetsPath,"scripts","pathfinder.scpt"),e=await v(r,{language:"JXA",stderrCallback:t=>y("Path Finder Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function me(){let r=P.default.join(I.environment.assetsPath,"scripts","qspace.scpt"),e=await v(r,{language:"JXA",stderrCallback:t=>y("QSpace Pro Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function ge(){let r=P.default.join(I.environment.assetsPath,"scripts","forklift-beta.scpt"),e=await v(r,{language:"JXA",stderrCallback:t=>y("ForkLift Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}var X=async(r,e)=>{let t=f.default.join(g.tmpdir(),`${r}.${e}`);return{path:t,[Symbol.asyncDispose]:async()=>{p.existsSync(t)&&await p.promises.rm(t,{recursive:!0})}}};var ye=async()=>{let e=(await c.LocalStorage.getItem("itemsToRemove")??"").toString().split(", ");for(let t of e)p.existsSync(t)&&await p.promises.rm(t,{recursive:!0});await c.LocalStorage.removeItem("itemsToRemove")},$e=async()=>{let r=[],t=(0,c.getPreferenceValues)().inputMethod,a=!1;if(t=="Clipboard")try{let n=(await le()).split(", ").reduce((o,l,u)=>(u==0||l.startsWith("/")?o.push(l):o[o.length-1]=`${o.at(-1)??""}${l}`,o),[]);if(await c.LocalStorage.setItem("itemsToRemove",n.join(", ")),n.filter(o=>o.trim().length>0).length>0)return n}catch(n){console.error(`Couldn't get images from clipboard: ${n}`),a=!0}let s=t;try{s=(await(0,c.getFrontmostApplication)()).name}catch(n){console.error(`Couldn't get frontmost application: ${n}`)}try{(t=="Path Finder"||s=="Path Finder")&&(r=await he())}catch(n){console.error(`Couldn't get images from Path Finder: ${n}`),a=!0}try{(t=="NeoFinder"||s=="NeoFinder")&&(r=await pe())}catch(n){console.error(`Couldn't get images from NeoFinder: ${n}`),a=!0}try{(t=="HoudahSpot"||s=="HoudahSpot")&&(r=await de())}catch(n){console.error(`Couldn't get images from HoudahSpot: ${n}`),a=!0}try{(t=="QSpace"||s=="QSpace Pro"||s=="QSpace")&&(r=await me())}catch(n){console.error(`Couldn't get images from ${s}: ${n}`),a=!0}try{(t=="ForkLift"||s=="ForkLift")&&(r=await ge())}catch(n){console.error(`Couldn't get images from ForkLift: ${n}`),a=!0}if(r.length>0)return r.filter((n,o)=>r.indexOf(n)===o);let i=await fe();return s=="Finder"||t=="Finder"||a?r=i:i.forEach(n=>{n.split("/").at(-2)=="Desktop"&&!r.includes(n)&&r.push(n)}),r.filter((n,o)=>r.indexOf(n)===o)},Z=async r=>{let e="Finder";try{e=(await(0,c.getFrontmostApplication)()).name}catch(a){console.error(`Couldn't get frontmost application: : ${a}`)}let t=(0,c.getPreferenceValues)();t.imageResultHandling=="copyToClipboard"?(await ue(r),be(r)):t.imageResultHandling=="openInPreview"?(await Ge(r),be(r)):t.inputMethod=="NeoFinder"||e=="NeoFinder"?await(0,c.showInFinder)(r[0]):(t.inputMethod=="HoudahSpot"||e=="HoudahSpot")&&await(0,c.showInFinder)(r[0])},Be=async()=>(g.cpus()[0].model.includes("Apple")?"arm":"x86")=="arm"?((0,k.execSync)(`chmod +x ${c.environment.assetsPath}/webp/arm/dwebp`),(0,k.execSync)(`chmod +x ${c.environment.assetsPath}/webp/arm/cwebp`),p.existsSync(`${c.environment.assetsPath}/webp/x86/dwebp`)&&await p.promises.rm(`${c.environment.assetsPath}/webp/x86/dwebp`),p.existsSync(`${c.environment.assetsPath}/webp/x86/cwebp`)&&await p.promises.rm(`${c.environment.assetsPath}/webp/x86/cwebp`),[`${c.environment.assetsPath}/webp/arm/dwebp`,`${c.environment.assetsPath}/webp/arm/cwebp`]):((0,k.execSync)(`chmod +x ${c.environment.assetsPath}/webp/x86/dwebp`),(0,k.execSync)(`chmod +x ${c.environment.assetsPath}/webp/x86/cwebp`),p.existsSync(`${c.environment.assetsPath}/webp/arm/dwebp`)&&await p.promises.rm(`${c.environment.assetsPath}/webp/arm/dwebp`),p.existsSync(`${c.environment.assetsPath}/webp/arm/cwebp`)&&await p.promises.rm(`${c.environment.assetsPath}/webp/arm/cwebp`),[`${c.environment.assetsPath}/webp/x86/dwebp`,`${c.environment.assetsPath}/webp/x86/cwebp`]),we=async(r,e)=>{var o=[];try{let t=(0,c.getPreferenceValues)();let a=O(o,await X("tmp","png"),!0);let s=(await C([e]))[0];let[i,n]=await Be();(0,k.execSync)(`${i} "${e}" -o "${a.path}" && ${r} "${a.path}" && ${n} ${t.useLosslessConversion?"-lossless":""} "${a.path}" -o "${s}"`);return s}catch(l){var u=l,b=!0}finally{var d=_(o,u,b);d&&await d}},ke=async(r,e)=>{var o=[];try{let t=(0,c.getPreferenceValues)();let a=O(o,await X("tmp","png"),!0);let s=(await C([e]))[0];let{encoderPath:i,decoderPath:n}=await J();(0,k.execSync)(`${n} "${e}" "${a.path}" && ${r} "${a.path}" && ${i} ${t.useLosslessConversion?"-s 0 --min 0 --max 0 --minalpha 0 --maxalpha 0 --qcolor 100 --qalpha 100":""}  "${a.path}" "${s}"`);return s}catch(l){var u=l,b=!0}finally{var d=_(o,u,b);d&&await d}},Se=async(r,e)=>{var s=[];try{let t=O(s,await X("tmp","bmp"),!0);let a=(await C([e]))[0];await He("BMP",e,t.path);(0,k.execSync)(`chmod +x ${c.environment.assetsPath}/potrace/potrace`);(0,k.execSync)(`${r} "${t.path}" && ${c.environment.assetsPath}/potrace/potrace -s --tight -o "${a}" "${t.path}"`);return a}catch(i){var n=i,o=!0}finally{var l=_(s,n,o);l&&await l}},He=async(r,e,t)=>$(`use framework "Foundation"
  use scripting additions
  ...
  pngData's writeToFile:"${t}" atomically:false`);var xe=async(r,e)=>{let t=(0,c.getPreferenceValues)(),a=r;t.imageResultHandling=="saveToDownloads"?a=f.default.join(g.homedir(),"Downloads",f.default.basename(a,f.default.extname(a))+".pdf"):t.imageResultHandling=="saveToDesktop"?a=f.default.join(g.homedir(),"Desktop",f.default.basename(a,f.default.extname(a))+".pdf"):(t.imageResultHandling=="copyToClipboard"||t.imageResultHandling=="openInPreview")&&(a=f.default.join(g.tmpdir(),f.default.basename(a,f.default.extname(a))+".pdf"));let s=2;for(;p.existsSync(a)&&g.tmpdir()!=f.default.dirname(a);)a=f.default.join(f.default.dirname(a),f.default.basename(a,f.default.extname(a))+` (${s})${f.default.extname(a)}`),s++;return $(`use framework "Foundation"
  use framework "PDFKit"
  ...
  rotatedPdfData's writeToFile:"${a}" atomically:false`),a};var Ge=async r=>{let e=Array.isArray(r)?r:[r],t=e.some(a=>f.default.extname(a)==".svg");await $(`use framework "Foundation"
    use scripting additions
  ...
  end tell`)},C=async(r,e=!1,t=void 0)=>{let a=(0,c.getPreferenceValues)(),s=await Je(r[0]);return r.map(i=>{let n=i;if(a.imageResultHandling=="saveToDownloads"?n=f.default.join(g.homedir(),"Downloads",f.default.basename(n)):a.imageResultHandling=="saveToDesktop"?n=f.default.join(g.homedir(),"Desktop",f.default.basename(n)):(a.imageResultHandling=="saveInContainingFolder"||a.imageResultHandling=="replaceOriginal")&&(a.inputMethod=="Clipboard"||e)?n=f.default.join(s,f.default.basename(n)):(a.imageResultHandling=="copyToClipboard"||a.imageResultHandling=="openInPreview")&&(n=f.default.join(g.tmpdir(),f.default.basename(n))),n=t?n.replace(f.default.extname(n),`.${t}`):n,a.imageResultHandling!="replaceOriginal"&&g.tmpdir()!=f.default.dirname(n)){let o=2;for(;p.existsSync(n);)n=f.default.join(f.default.dirname(n),f.default.basename(n,f.default.extname(n))+`-${o}${f.default.extname(n)}`),o++}return n})},y=async(r,e,t,a)=>{console.error(e),t?(t.title=r,t.message=a??e.message,t.style=c.Toast.Style.Failure,t.primaryAction={title:"Copy Error",onAction:async()=>{await c.Clipboard.copy(e.message)}}):t=await(0,c.showToast)({title:r,message:a??e.message,style:c.Toast.Style.Failure,primaryAction:{title:"Copy Error",onAction:async()=>{await c.Clipboard.copy(e.message)}}})};var ve=r=>{let e=g.homedir();if(r.startsWith("~"))return r.replace(/^~(?=$|\/|\\)/,e);let t=/(\/Users\/.*?)\/.*/,a=r.match(t);return a?r.replace(a[1],e):r};async function q(r,e){let t=r.map(i=>ve(i)),a='"'+t.join('" "')+'"',s=await C(t);if(a.toLowerCase().includes("webp")||a.toLowerCase().includes("svg")||a.toLowerCase().includes("pdf")||a.toLowerCase().includes("avif")){let i=[];for(let n of t)if(n.toLowerCase().endsWith("webp"))i.push(await we(`sips --rotate ${e}`,n));else if(n.toLowerCase().endsWith("svg"))i.push(await Se(`sips --rotate ${e}`,n));else if(n.toLowerCase().endsWith("pdf"))i.push(await xe(n,e));else if(n.toLowerCase().endsWith("avif"))i.push(await ke(`sips --rotate ${e}`,n));else{let o=s[t.indexOf(n)];i.push(o),(0,W.execSync)(`sips --rotate ${e} -o "${o}" "${n}"`)}return await Z(i),i}else{if(s.length==1)(0,W.execSync)(`sips --rotate ${e} -o "${s[0]}" ${a}`);else{let i=D.default.join(D.default.dirname(s[0]),"rotated"),n=1;for(;A.existsSync(i);)i=i+`-${n}`,n++;await A.promises.mkdir(i,{recursive:!0}),(0,W.execSync)(`sips --rotate ${e} -o "${i}" ${a}`),t.forEach((o,l)=>{A.renameSync(D.default.join(i,D.default.basename(o)),s[l])}),await A.promises.rm(i,{recursive:!0,force:!0})}await Z(s)}return s}async function Xe({angle:r,isRadians:e,imagePaths:t}){let a=parseFloat(r.toString());if(isNaN(a))throw new Error("Invalid angle: must be a number");e===!0&&(a=a*(180/Math.PI));let s=t?.length?t:await $e(),i=await q(s,a);return await ye(),i}
