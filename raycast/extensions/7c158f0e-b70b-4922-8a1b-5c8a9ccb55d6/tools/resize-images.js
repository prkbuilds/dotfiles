...
`),process.nextTick(()=>u.stdin.uncork()),e?.logSentMessages&&console.log(`Sent message: ${f}`))},{data:(async()=>new Promise((f,E)=>{let V=e?.timeout?setTimeout(()=>{try{u.kill()}catch(x){e?.logErrors&&console.error(`Error killing process: ${x}`)}return e?.logErrors&&console.error("Script timed out"),u.stdin.end(),u.kill(),E("Script timed out")},e?.timeout):void 0;u.on("close",x=>{if(x!==0)return e?.logErrors&&console.error(`Script exited with code ${x}`),u.stdin.end(),u.kill(),E(`Script exited with code ${x}`);clearTimeout(V);let R;try{R=JSON.parse(i)}catch{R=i.trim()}return e?.logFinalOutput&&console.log(`Script output: ${R}`),f(R)})}))(),sendMessage:l}}var A=require("@raycast/api");async function he(){let r=I.default.join(A.environment.assetsPath,"scripts","finder.scpt"),e=await P(r,{language:"AppleScript",stderrCallback:t=>b("Finder Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function me(){let r=I.default.join(A.environment.assetsPath,"scripts","houdahSpot.scpt"),e=await P(r,{language:"AppleScript",stderrCallback:t=>b("HoudahSpot Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function ge(){let r=I.default.join(A.environment.assetsPath,"scripts","neofinder.scpt"),e=await P(r,{language:"AppleScript",stderrCallback:t=>b("NeoFinder Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function be(){let r=I.default.join(A.environment.assetsPath,"scripts","pathfinder.scpt"),e=await P(r,{language:"JXA",stderrCallback:t=>b("Path Finder Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function ye(){let r=I.default.join(A.environment.assetsPath,"scripts","qspace.scpt"),e=await P(r,{language:"JXA",stderrCallback:t=>b("QSpace Pro Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}async function $e(){let r=I.default.join(A.environment.assetsPath,"scripts","forklift-beta.scpt"),e=await P(r,{language:"JXA",stderrCallback:t=>b("ForkLift Selection Error",new Error(t))}).data;return Array.isArray(e)?e:e.split(",").map(t=>t.trim())}var q=async(r,e)=>{let t=d.default.join(y.tmpdir(),`${r}.${e}`);return{path:t,[Symbol.asyncDispose]:async()=>{p.existsSync(t)&&await p.promises.rm(t,{recursive:!0})}}};var ke=async()=>{let e=(await c.LocalStorage.getItem("itemsToRemove")??"").toString().split(", ");for(let t of e)p.existsSync(t)&&await p.promises.rm(t,{recursive:!0});await c.LocalStorage.removeItem("itemsToRemove")},Se=async()=>{let r=[],t=(0,c.getPreferenceValues)().inputMethod,a=!1;if(t=="Clipboard")try{let n=(await de()).split(", ").reduce((i,l,u)=>(u==0||l.startsWith("/")?i.push(l):i[i.length-1]=`${i.at(-1)??""}${l}`,i),[]);if(await c.LocalStorage.setItem("itemsToRemove",n.join(", ")),n.filter(i=>i.trim().length>0).length>0)return n}catch(n){console.error(`Couldn't get images from clipboard: ${n}`),a=!0}let s=t;try{s=(await(0,c.getFrontmostApplication)()).name}catch(n){console.error(`Couldn't get frontmost application: ${n}`)}try{(t=="Path Finder"||s=="Path Finder")&&(r=await be())}catch(n){console.error(`Couldn't get images from Path Finder: ${n}`),a=!0}try{(t=="NeoFinder"||s=="NeoFinder")&&(r=await ge())}catch(n){console.error(`Couldn't get images from NeoFinder: ${n}`),a=!0}try{(t=="HoudahSpot"||s=="HoudahSpot")&&(r=await me())}catch(n){console.error(`Couldn't get images from HoudahSpot: ${n}`),a=!0}try{(t=="QSpace"||s=="QSpace Pro"||s=="QSpace")&&(r=await ye())}catch(n){console.error(`Couldn't get images from ${s}: ${n}`),a=!0}try{(t=="ForkLift"||s=="ForkLift")&&(r=await $e())}catch(n){console.error(`Couldn't get images from ForkLift: ${n}`),a=!0}if(r.length>0)return r.filter((n,i)=>r.indexOf(n)===i);let o=await he();return s=="Finder"||t=="Finder"||a?r=o:o.forEach(n=>{n.split("/").at(-2)=="Desktop"&&!r.includes(n)&&r.push(n)}),r.filter((n,i)=>r.indexOf(n)===i)},Q=async r=>{let e="Finder";try{e=(await(0,c.getFrontmostApplication)()).name}catch(a){console.error(`Couldn't get frontmost application: : ${a}`)}let t=(0,c.getPreferenceValues)();t.imageResultHandling=="copyToClipboard"?(await pe(r),we(r)):t.imageResultHandling=="openInPreview"?(await Be(r),we(r)):t.inputMethod=="NeoFinder"||e=="NeoFinder"?await(0,c.showInFinder)(r[0]):(t.inputMethod=="HoudahSpot"||e=="HoudahSpot")&&await(0,c.showInFinder)(r[0])},ze=async()=>(y.cpus()[0].model.includes("Apple")?"arm":"x86")=="arm"?((0,k.execSync)(`chmod +x ${c.environment.assetsPath}/webp/arm/dwebp`),(0,k.execSync)(`chmod +x ${c.environment.assetsPath}/webp/arm/cwebp`),p.existsSync(`${c.environment.assetsPath}/webp/x86/dwebp`)&&await p.promises.rm(`${c.environment.assetsPath}/webp/x86/dwebp`),p.existsSync(`${c.environment.assetsPath}/webp/x86/cwebp`)&&await p.promises.rm(`${c.environment.assetsPath}/webp/x86/cwebp`),[`${c.environment.assetsPath}/webp/arm/dwebp`,`${c.environment.assetsPath}/webp/arm/cwebp`]):((0,k.execSync)(`chmod +x ${c.environment.assetsPath}/webp/x86/dwebp`),(0,k.execSync)(`chmod +x ${c.environment.assetsPath}/webp/x86/cwebp`),p.existsSync(`${c.environment.assetsPath}/webp/arm/dwebp`)&&await p.promises.rm(`${c.environment.assetsPath}/webp/arm/dwebp`),p.existsSync(`${c.environment.assetsPath}/webp/arm/cwebp`)&&await p.promises.rm(`${c.environment.assetsPath}/webp/arm/cwebp`),[`${c.environment.assetsPath}/webp/x86/dwebp`,`${c.environment.assetsPath}/webp/x86/cwebp`]),W=async(r,e)=>{var i=[];try{let t=(0,c.getPreferenceValues)();let a=O(i,await q("tmp","png"),!0);let s=(await C([e]))[0];let[o,n]=await ze();(0,k.execSync)(`${o} "${e}" -o "${a.path}" && ${r} "${a.path}" && ${n} ${t.useLosslessConversion?"-lossless":""} "${a.path}" -o "${s}"`);return s}catch(l){var u=l,g=!0}finally{var f=_(i,u,g);f&&await f}},M=async(r,e)=>{var i=[];try{let t=(0,c.getPreferenceValues)();let a=O(i,await q("tmp","png"),!0);let s=(await C([e]))[0];let{encoderPath:o,decoderPath:n}=await Z();(0,k.execSync)(`${n} "${e}" "${a.path}" && ${r} "${a.path}" && ${o} ${t.useLosslessConversion?"-s 0 --min 0 --max 0 --minalpha 0 --maxalpha 0 --qcolor 100 --qalpha 100":""}  "${a.path}" "${s}"`);return s}catch(l){var u=l,g=!0}finally{var f=_(i,u,g);f&&await f}},j=async(r,e)=>{var s=[];try{let t=O(s,await q("tmp","bmp"),!0);let a=(await C([e]))[0];await He("BMP",e,t.path);(0,k.execSync)(`chmod +x ${c.environment.assetsPath}/potrace/potrace`);(0,k.execSync)(`${r} "${t.path}" && ${c.environment.assetsPath}/potrace/potrace -s --tight -o "${a}" "${t.path}"`);return a}catch(o){var n=o,i=!0}finally{var l=_(s,n,i);l&&await l}},He=async(r,e,t)=>w(`use framework "Foundation"
  ...
  end tell`)},C=async(r,e=!1,t=void 0)=>{let a=(0,c.getPreferenceValues)(),s=await Ke(r[0]);return r.map(o=>{let n=o;if(a.imageResultHandling=="saveToDownloads"?n=d.default.join(y.homedir(),"Downloads",d.default.basename(n)):a.imageResultHandling=="saveToDesktop"?n=d.default.join(y.homedir(),"Desktop",d.default.basename(n)):(a.imageResultHandling=="saveInContainingFolder"||a.imageResultHandling=="replaceOriginal")&&(a.inputMethod=="Clipboard"||e)?n=d.default.join(s,d.default.basename(n)):(a.imageResultHandling=="copyToClipboard"||a.imageResultHandling=="openInPreview")&&(n=d.default.join(y.tmpdir(),d.default.basename(n))),n=t?n.replace(d.default.extname(n),`.${t}`):n,a.imageResultHandling!="replaceOriginal"&&y.tmpdir()!=d.default.dirname(n)){let i=2;for(;p.existsSync(n);)n=d.default.join(d.default.dirname(n),d.default.basename(n,d.default.extname(n))+`-${i}${d.default.extname(n)}`),i++}return n})},b=async(r,e,t,a)=>{console.error(e),t?(t.title=r,t.message=a??e.message,t.style=c.Toast.Style.Failure,t.primaryAction={title:"Copy Error",onAction:async()=>{await c.Clipboard.copy(e.message)}}):t=await(0,c.showToast)({title:r,message:a??e.message,style:c.Toast.Style.Failure,primaryAction:{title:"Copy Error",onAction:async()=>{await c.Clipboard.copy(e.message)}}})};var xe=r=>{let e=y.homedir();if(r.startsWith("~"))return r.replace(/^~(?=$|\/|\\)/,e);let t=/(\/Users\/.*?)\/.*/,a=r.match(t);return a?r.replace(a[1],e):r};async function ee(r,e,t){let a=r.map(n=>xe(n)),s='"'+a.join('" "')+'"',o=await C(a);if(s.toLocaleLowerCase().includes("webp")||s.toLocaleLowerCase().includes("svg")||s.toLocaleLowerCase().includes("avif")){let n=[];for(let i of a)if(i.toLowerCase().endsWith(".webp"))e!=-1&&t==-1?n.push(await W(`sips --resampleWidth ${e}`,i)):e==-1&&t!=-1?n.push(await W(`sips --resampleHeight ${t}`,i)):n.push(await W(`sips --resampleHeightWidth ${t} ${e}`,i));else if(i.toLowerCase().endsWith(".svg"))e!=-1&&t==-1?n.push(await j(`sips --resampleWidth ${e}`,i)):e==-1&&t!=-1?n.push(await j(`sips --resampleHeight ${t}`,i)):n.push(await j(`sips --resampleHeightWidth ${t} ${e}`,i));else if(i.toLowerCase().endsWith(".avif"))e!=-1&&t==-1?n.push(await M(`sips --resampleWidth ${e}`,i)):e==-1&&t!=-1?n.push(await M(`sips --resampleHeight ${t}`,i)):n.push(await M(`sips --resampleHeightWidth ${t} ${e}`,i));else{let l=o[a.indexOf(i)];n.push(l),e!=-1&&t==-1?(0,S.execSync)(`sips --resampleWidth ${e} -o "${l}" "${i}"`):e==-1&&t!=-1?(0,S.execSync)(`sips --resampleHeight ${t} -o "${l}" "${i}"`):(0,S.execSync)(`sips --resampleHeightWidth ${t} -o "${l}" ${e} "${i}"`)}return await Q(n),n}else{let n=o.length==1?o[0]:D.default.join(D.default.dirname(o[0]),"resized");o.length>1&&(0,S.execSync)(`mkdir -p "${n}"`),e!=-1&&t==-1?(0,S.execSync)(`sips --resampleWidth ${e} -o "${n}" ${s}`):e==-1&&t!=-1?(0,S.execSync)(`sips --resampleHeight ${t} -o "${n}" ${s}`):(0,S.execSync)(`sips --resampleHeightWidth ${t} ${e} -o "${n}" ${s}`),o.length>1&&(await Promise.all(a.map((i,l)=>Y.default.promises.rename(D.default.join(n,D.default.basename(i)),o[l]))),await Y.default.promises.rm(n,{recursive:!0,force:!0})),await Q(o)}return o}async function Je({width:r,height:e,imagePaths:t}){let a=parseInt(r.toString()),s=parseInt(e.toString());if(isNaN(a)||s!==-1&&a<1)throw new Error("Invalid width: must be a positive integer");if(isNaN(s)||s!==-1&&s<1)throw new Error("Invalid height: must be a positive integer");let o=t?.length?t:await Se(),n=await ee(o,a,s);return await ke(),n}
